        ...
        <form @submit.prevent="editRoom">
          <div class="flex flex-col mb-4">
            ...
            <div
              ...
              <input class="bg-transparent border-none flex-1 outline-none" type="text" v-model="customerDetail.name"
                required />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            ...
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input class="bg-transparent border-none flex-1 outline-none" type="text" v-model="customerDetail.cardId"
                required disabled />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            ...
            <div
              ...
              <input class="bg-transparent border-none flex-1 outline-none" type="text"
                v-model="customerDetail.phoneNumber" required />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            <label class="text-lg font-medium text-gray-700 mb-1">
            ...
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input class="bg-transparent border-none flex-1 outline-none" type="text" v-model="customerDetail.email"
                required />
            </div>
          </div>
    ...
    axiosInstance
      ...
      .then((response) => {
        ...
        this.shortData = response.data.map(
          ({ id, name, cardId, phoneNumber, email }) => ({
            id,
            name,
            cardId,
            phoneNumber,
  ...
  methods: {
    searchData() {
      ...
      this.filteredData = this.shortData.filter((item) => {
        ...
        if (this.searchTerm.length >= 1) {
          ...
          if (!isNaN(prefix)) {
            ...
            if (item.cardId) {
              ...
              const cardIdStr = item.cardId.toString();
              if (cardIdStr.includes(prefix)) {
                return true;
              }
            }
          }
        ...
        const cardIdMatch = item.cardId && item.cardId === searchTermInt;
    ...
    showEditModal(customerId) {
      ...
      const selectedCustomer = this.shortData.find(
        (item) => item.id === customerId
      );
      this.customerDetail = { ...selectedCustomer };
    },
    editRoom() {
      const customerId = this.customerIdEdit;
      ...
      const customerData = {
        ...
        cardId: this.customerDetail.cardId,
      };

      // Gửi request axios để sửa phòng với customerId
      axiosInstance
        .post(`/client/save`, customerData) // Gửi dữ liệu dưới dạng JSON
        .then(() => {
          // Gửi request để lấy lại toàn bộ danh sách phòng sau khi sửa thành công
          return axiosInstance.post("/client/getall");
        })
        .then((response) => {
          this.shortData = response.data.map(
            ({ id, name, cardId, phoneNumber, email }) => ({
          ...
          this.customerIdEdit = null;
        })
        .catch((error) => {
          // Xử lý lỗi nếu có
          alert("Lỗi khi sửa thông tin khách hàng:", error);
        });
    },
  },
  computed: {
