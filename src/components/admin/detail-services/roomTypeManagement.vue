  ...
  <div class="roomManagement">
    ...
    <div class="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex" v-if="addModal">
      ...
      <div class="modalContainer bg-white p-8 rounded-lg m-auto">
        <h2 class="text-2xl font-bold mb-8 uppercase">Thêm mới loại phòng</h2>
        <form @submit.prevent="addRoomType">
          <div class="flex flex-col mb-4">
            ...
            <div
              ...
              <input :placeholder="popupContent.roomTypeName" class="bg-transparent border-none flex-1 outline-none"
                type="text" v-model="newRoomType.typeName" required />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            <label class="text-lg font-medium text-gray-700 mb-1">
              {{ popupContent.capacity }}
            </label>
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input :placeholder="popupContent.capacity" class="bg-transparent border-none flex-1 outline-none"
                type="text" v-model="newRoomType.numofPeople" required />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            <label class="text-lg font-medium text-gray-700 mb-1">
              {{ popupContent.roomTypePrice }}
            </label>
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input :placeholder="popupContent.roomTypePrice" class="bg-transparent border-none flex-1 outline-none"
                type="text" v-model="newRoomType.price" required />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            <label class="text-lg font-medium text-gray-700 mb-1">
              {{ popupContent.descrip }}
            </label>
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input :placeholder="popupContent.descrip" class="bg-transparent border-none flex-1 outline-none"
                type="text" v-model="newRoomType.description" />
            </div>
          </div>
          <upload-image label="Tải Ảnh" @image-uploaded="handleImageUpload" @image-removed="handleImageRemove">
          </upload-image>
    ...
    <div class="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex" v-if="editModal">
      <div class="modalContainer bg-white p-8 rounded-lg m-auto">
        <h2 class="text-2xl uppercase font-bold mb-4">Sửa loại phòng</h2>
        <form @submit.prevent="editRoomType">
          <div class="flex flex-col mb-4">
            <label class="text-lg font-medium text-gray-700 mb-1">
              Mã Loại Phòng
            </label>
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input class="bg-transparent border-none flex-1 outline-none" type="text" v-model="roomTypeDetail.id"
                required disabled />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            ...
            <div
              ...
              <input class="bg-transparent border-none flex-1 outline-none" type="text"
                v-model="roomTypeDetail.typeName" required />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            <label class="text-lg font-medium text-gray-700 mb-1">
            ...
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input class="bg-transparent border-none flex-1 outline-none" type="text"
                v-model="roomTypeDetail.numofPeople" required />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            <label class="text-lg font-medium text-gray-700 mb-1">
            ...
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input class="bg-transparent border-none flex-1 outline-none" type="text" v-model="roomTypeDetail.price"
                required />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            ...
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input class="bg-transparent border-none flex-1 outline-none" type="text"
                v-model="roomTypeDetail.description" />
            </div>
          </div>
        ...
        <form>
          ...
          <div class="flex flex-col mb-4">
            ...
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input class="bg-transparent border-none flex-1 outline-none" type="text"
                v-model="roomTypeDetail.typeName" required disabled />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            <label class="text-lg font-medium text-gray-700 mb-1">
            ...
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input class="bg-transparent border-none flex-1 outline-none" type="text"
                v-model="roomTypeDetail.numofPeople" required disabled />
            </div>
          </div>
          <div class="flex flex-col mb-4">
            <label class="text-lg font-medium text-gray-700 mb-1">
          ...
          <div class="flex flex-col mb-4">
            ...
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input class="bg-transparent border-none flex-1 outline-none" type="text"
                v-model="roomTypeDetail.description" disabled />
            </div>
          </div>
          <div class="flex justify-end">
    ...
    <div class="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex" v-if="deleteModal">
      ...
      <div class="flex flex-col items-center justify-center bg-white p-8 rounded-lg m-auto">
        ...
        <form @submit.prevent="deleteRoomType">
...
<script>
import UploadImage from "../upload-image/upload-image.vue";
...
export default {
  ...
  data() {
    return {
      popupContent: {
        ...
        capacity: "Số Người Trong Phòng",
        roomTypePrice: "Giá Tiền(VNĐ)",
        descrip: "Mô Tả",
        editRoom: "Sửa",
        addRoom: "Thêm mới",
        calcelRoom: "Đóng",
      ...
      searchForm: "Mã loại phòng, tên loại phòng",
      operationSettings: {
        showEdit: true,
        showDelete: true,
        showAdd: true,
  ...
  methods: {
        ...
        if (this.searchTerm.length >= 1) {
          ...
          if (!isNaN(prefix)) {
            ...
            if (item.id) {
              ...
              const roomTypeStr = item.id.toString();
              if (roomTypeStr.includes(prefix)) {
                return true;
              }
            }
          }
    ...
    showEditModal(roomTypeId) {
      this.roomTypeIdEdit = roomTypeId;
      this.editModal = !this.editModal;
      const selectedRoomType = this.shortData.find(
        (roomType) => roomType.id === roomTypeId
      );
      this.roomTypeDetail = { ...selectedRoomType };
    },
    showDeleteModal(roomTypeId) {
      this.roomTypeIdEdit = roomTypeId;
      this.deleteModal = !this.deleteModal;
    },
    showViewModal(roomTypeId) {
      this.roomTypeIdEdit = roomTypeId;
      this.viewModal = !this.viewModal;
      this.roomTypeDetail = this.shortData.find(
        (service) => service.id === roomTypeId
      );
    },
    handleImageUpload(file) {
      this.newRoomType.image = file;
    },
    handleImageRemove() {
      this.newRoomType.image = null;
    },

    // editRoomType đã sửa
    editRoomType() {
      const roomTypeId = this.roomTypeIdEdit;

      // Tạo object chứa dữ liệu của service cần sửa
      const roomTypeData = {
        id: roomTypeId,
        typeName: this.roomTypeDetail.typeName,
        numofPeople: this.roomTypeDetail.numofPeople,
        description: this.roomTypeDetail.description,
        price: this.roomTypeDetail.price,
      };

      // Gửi request axios để sửa phòng với roomTypeId
      axiosInstance
        .post(`/roomtype/save`, roomTypeData) // Gửi dữ liệu dưới dạng JSON
        .then(() => {
          // Gửi request để lấy lại toàn bộ danh sách phòng sau khi sửa thành công
          return axiosInstance.post("/roomtype/getall");
        })
        .then((response) => {
          // Lọc và chỉ lấy những trường cần thiết cho mảng short
          this.shortData = response.data.map(
          ...
          this.roomTypeIdEdit = null;
        })
        .catch((error) => {
          // Xử lý lỗi nếu có
          alert("Lỗi khi sửa loại phòng:", error);
        });
    },
    ...
    deleteRoomType() {
      ...
      if (this.roomTypeIdEdit) {
        // Gửi request axios để xóa phòng với roomTypeId
        axiosInstance
          .post(`/roomtype/delete`, roomTypeData)
          .then(() => {
            // Gửi request để lấy lại toàn bộ danh sách phòng sau khi sửa thành công
            return axiosInstance.post("/roomtype/getall");
          })
          ...
          .catch((error) => {
            // Xử lý lỗi nếu có
            alert("Lỗi khi xóa loại phòng:", error);
          });
      } else {
        // Trường hợp roomTypeIdEdit không có giá trị
    ...
    addRoomType() {
      ...
      const roomTypeData = {
        typeName: this.newRoomType.typeName,
        numofPeople: this.newRoomType.numofPeople,
        description: this.newRoomType.description,
        price: this.newRoomType.price,
      };
      ...
      axiosInstance
        ...
        .catch((error) => {
          // Xử lý lỗi nếu có
          alert("Lỗi khi thêm mới loại phòng: ", error);
        });
    },
  },
  computed: {
