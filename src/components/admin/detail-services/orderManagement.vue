<template>
  <div class="roomManagement">
                ...
                <template v-if="key !== 'id'">
                  ...
                  <td v-if="key === 'isChecked'" :key="key"
                    ...
                    <div class="text-sm text-gray-900">
                      {{ value ? "Đã Nhận Phòng" : "Chưa Nhận Phòng" }}
                    </div>
                  </td>
                </template>
              </template>
    ...
    <div class="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex" v-if="editModal">
      <div class="modalContainer bg-white p-8 rounded-lg m-auto">
        <h2 class="text-2xl uppercase font-bold mb-4">
          Sửa đơn đặt phòng số {{ orderDetail.bookingId }}
        </h2>
        <form @submit.prevent="editOrder">
            ...
            <div class="flex flex-col mb-4 w-1/2">
              ...
              <div
                ...
                <input class="bg-transparent border-none flex-1 outline-none" type="text"
                  v-model="orderDetail.clientName" disabled />
              </div>
            </div>
            <div class="flex flex-col mb-4 w-1/2">
              <label class="text-lg font-medium text-gray-700 mb-1">
              ...
              <div
                class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
                <input class="bg-transparent border-none flex-1 outline-none" type="text"
                  v-model="orderDetail.clientCardId" required disabled />
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center justify-center space-x-3">
            <div class="flex flex-col mb-4 w-1/2">
              ...
              <div
                class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
                <input class="bg-transparent border-none flex-1 outline-none" type="text" v-model="orderDetail.checkin"
                  required disabled />
              </div>
            </div>
            <div class="flex flex-col mb-4 w-1/2">
              ...
              <div
                class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
                <input class="bg-transparent border-none flex-1 outline-none" type="text" v-model="orderDetail.checkout"
                  required disabled />
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center justify-center space-x-3">
            <div class="flex flex-col mb-4 w-1/2">
              ...
              <div class="relative">
                <template v-if="!orderDetail.isChecked">
                  <div class="absolute z-10 top-[38px] mt-2 w-full border border-gray-300 rounded-lg shadow-lg bg-white"
                    ...
                    <ul class="max-h-60 overflow-y-auto rounded-lg">
                      <li v-for="item in filterRoomCodes()" :key="item.id" class="px-4 py-2 hover:bg-gray-100"
                        @click="selectRoomCode(item)" :class="{
        'cursor-pointer': orderDetail.isChecked === false,
        'cursor-normal': orderDetail.isChecked === true,
        'hover:bg-gray-100': !isSelectedRoomCode(item),
        'bg-gray-100': isSelectedRoomCode(item),
      }">
                        {{ item.roomCode }}
                      </li>
                    </ul>
                ...
                <div
                  ...
                  <span class="flex" v-if="orderDetail.isChecked === false">
                    {{ selectedRoomCode || orderDetail.roomCode }}
                  </span>
                  <span class="flex" v-else>
                    <input class="bg-transparent border-none flex-1 outline-none" type="text"
                      v-model="orderDetail.roomCode" required :disabled="orderDetail.isChecked === 'true'"
                      :readonly="orderDetail.isChecked === 'true'" />
                  </span>
                  <svg v-if="orderDetail.isChecked === false" class="fill-current h-5 w-5"
            ...
            <div class="flex flex-col mb-4 w-1/2">
              ...
              <div class="flex items-center justify-center">
                <div class="flex items-center justify-start w-1/2">
                  <label>
                    <input type="radio" name="status" v-model="orderDetail.isChecked" :value="true" />
                    Đã Nhận Phòng
                  </label>
                </div>
                <div class="flex items-center justify-start w-1/2">
                  <label>
                    <input type="radio" v-model="orderDetail.isChecked" :value="false" name="status" />
                    Chưa Nhận Phòng
                  </label>
                </div>
              </div>
          ...
          <div class="flex items-center justify-center space-x-3">
            <div class="flex flex-col mb-4 w-1/2">
              ...
              <div
                class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
                <input class="bg-transparent border-none flex-1 outline-none" type="text"
                  v-model="orderDetail.roomFloor" required disabled />
              </div>
            </div>
            <div class="flex flex-col mb-4 w-1/2">
              <label class="text-lg font-medium text-gray-700 mb-1">
              ...
              <div
                class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
                <input class="bg-transparent border-none flex-1 outline-none" type="text"
                  v-model="orderDetail.roomTypeName" required disabled />
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-col mb-4">
            ...
            <div
              class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
              <input class="bg-transparent border-none flex-1 outline-none" type="text" v-model="orderDetail.note"
                required disabled />
            </div>
          </div>
    ...
    <div class="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex" v-if="viewModal">
      <div class="modalContainer bg-white p-8 rounded-lg m-auto">
        <h2 class="text-2xl uppercase font-bold mb-4">
          Chi tiết đơn đặt phòng số {{ orderDetail.bookingId }}
        </h2>
        <form>
          <div class="flex items-center justify-center space-x-3">
            <div class="flex flex-col mb-4 w-1/2">
          ...
          <div class="flex items-center justify-center space-x-3">
            <div class="flex flex-col mb-4 w-1/2">
              ...
              <div
                class="px-4 py-2 w-full text-base divForm ring-1 leading-[150%] rounded-lg text-[#1C1F23] bg-[#F6F9FC]">
                <input class="bg-transparent border-none flex-1 outline-none" type="text" v-model="orderDetail.roomCode"
                  required disabled />
              </div>
            </div>
            <div class="flex flex-col mb-4 w-1/2">
              ...
              <div class="flex items-center justify-center">
                <div class="flex items-center justify-start w-1/2">
                  <label>
                    <input type="radio" name="status" v-model="orderDetail.isChecked" value="true" disabled />
                    Đã Nhận Phòng
                  </label>
                </div>
                <div class="flex items-center justify-start w-1/2">
                  <label>
                    <input type="radio" v-model="orderDetail.isChecked" value="false" name="status" disabled />
                    Chưa Nhận Phòng
                  </label>
                </div>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      roomVarcant: [], // Mảng lưu danh sách các loại phòng
      orderDetail: {
        roomID: "",
        roomCode: "",
        isChecked: null, // Khởi tạo isChecked
      },
    };
  },
  mounted() {
    if (this.orderDetail && this.orderDetail.roomCode) {
      // Gán giá trị từ BE cho selectedRoomCode khi dữ liệu được tải lên từ backend
      this.selectedRoomCode = this.orderDetail.roomCode;
    }
  },
  created() {
    // Sử dụng instance đã được tạo với interceptor
    Promise.all([
      axiosInstance.post("/bookedroom/getall"),
      axiosInstance.post("/room/getall"),
    ])
      .then((responses) => {
        // Xử lý dữ liệu từ response của yêu cầu lấy thông tin phòng
        this.fullData = responses[0].data.map(
          ({
            id,
            bookingId,
            clientName,
            ...
            clientCardId,
            roomCode,
            checkin: moment(checkin).format("DD/MM/YYYY - HH:mm:ss"),
            checkout: moment(checkout).format("DD/MM/YYYY - HH:mm:ss"),
            roomFloor,
            roomTypeName,
            isChecked,
            roomID,
        ...
        this.shortData = responses[0].data.map(
          ({
            id,
            bookingId,
            clientName,
        ...
        this.roomVarcant = responses[1].data.map(
          ({ id, roomCode, roomStatus, roomTypeId }) => ({
            id,
            roomCode,
            roomStatus,
            roomTypeId,
  ...
  methods: {
      ...
      this.filteredData = this.shortData.filter((item) => {
        ...
        if (this.searchTerm.length >= 1) {
          ...
          if (!isNaN(prefix)) {
            ...
            if (item.clientCardId) {
              // So sánh roomCode với prefix
              const identityCardStr = item.clientCardId.toString();
        ...
        const identityCardMatch =
          item.clientCardId && item.clientCardId === searchTermInt;
    ...
    showEditModal(orderId) {
      ...
      this.orderIdEdit = orderId;
      this.editModal = !this.editModal;
      const selectedOrder = this.fullData.find((order) => order.id === orderId);
      this.orderDetail = { ...selectedOrder };
    },
    showViewModal(orderId) {
      this.orderIdEdit = orderId;
      this.viewModal = !this.viewModal;
      this.orderDetail = this.fullData.find((order) => order.id === orderId);
    },
    editOrder() {
      const orderId = this.orderIdEdit;
      const isChecked = this.orderDetail.isChecked === true;

      // Tạo object chứa dữ liệu của phòng cần sửa
      const orderData = {
        id: orderId,
        roomid: this.orderDetail.roomID,
        ischecked: isChecked,
      };
      // Gửi request axios để sửa phòng với orderId
      axiosInstance
        .post(`/bookedroom/Save`, orderData) // Gửi dữ liệu dưới dạng JSON
        .then(() => {
        ...
        .then((response) => {
          this.fullData = response[0].data.map(
            ({
              id,
              bookingId,
              clientName,
          ...
          // Lọc và chỉ lấy những trường cần thiết cho mảng short
          this.shortData = response[0].data.map(
            ({
              id,
              bookingId,
              clientName,
          ...
          this.roomVarcant = response[1].data.map(
            ({ id, roomCode, roomStatus, roomTypeId }) => ({
              id,
              roomCode,
              roomStatus,
          ...
          alert("Sửa đơn đặt thành công");
          this.editModal = false;
          this.orderIdEdit = null;
        })
        .catch((error) => {
          // Xử lý lỗi nếu có
          alert("Lỗi khi sửa đơn đặt:", error);
        });
    },
    filterRoomCodes() {
      // Lọc các phòng có cùng roomTypeId và roomStatus là 'Varcant'
      return this.roomVarcant.filter(
        (item) =>
          item.roomTypeId === this.orderDetail.roomTypeId &&
          item.roomStatus === "Varcant"
      );
    },
    toggleDropdown() {
    ...
    selectRoomCode(item) {
      this.selectedRoomCode = item.roomCode;
      this.orderDetail.roomID = item.id;
      this.orderDetail.roomCode = item.roomCode;
      this.isDropdownOpen = false;
    },
    isSelectedRoomCode(item) {
      this.roomVarcant.roomCode = item.roomCode;
    },
  },
  computed: {
    paginated() {
